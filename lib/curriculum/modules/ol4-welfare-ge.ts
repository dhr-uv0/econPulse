...
The compensation principle also has a philosophical dimension: should hypothetical compensation suffice for a welfare judgement? Rawlsians argue no — actual redistribution to losers is required for a policy to be just. This links Pareto and KH analysis to normative political philosophy. The "potential Pareto improvement" language used in cost-benefit analysis papers precisely sidesteps this debate by noting only that gainers could compensate losers in principle, leaving the political question of actual redistribution to elected decision-makers. For olympiad purposes, students should master the formal definitions, recognise the Scitovsky paradox by construction, and be able to compare how different social welfare functions would rank allocations on the UPF.`,
      ...
      thinkAboutIt: {
        question: `Suppose x* and y* are both Pareto-efficient allocations of a fixed endowment between two agents A and B, with x* ≠ y* and u_A(x*) > u_A(y*) while u_B(x*) < u_B(y*). Can the Pareto criterion alone tell us which of x* or y* society should prefer? Prove your answer directly from the definition of Pareto efficiency, and explain what this implies about the Pareto frontier as a locus of allocations.`,
        answer: `No — the Pareto criterion is silent between any two distinct Pareto-efficient allocations. Suppose for contradiction that x* Pareto-dominates y*: this requires u_i(x*) ≥ u_i(y*) for both agents with strict inequality for at least one. But if that held, y* would admit a Pareto improvement (namely x*), contradicting the assumption that y* is itself Pareto efficient. By the symmetric argument, y* cannot dominate x* either. Hence any two distinct Pareto-efficient allocations are Pareto-incomparable — neither is a Pareto improvement over the other — which is precisely why the entire utility possibility frontier consists of mutually unranked points. This proves formally that Pareto efficiency defines only a partial order over allocations, not a complete one, so selecting a single point on the UPF (as opposed to merely ruling out interior, dominated points) necessarily requires an auxiliary social welfare function that embeds explicit value judgements about interpersonal trade-offs.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `Two agents A and B consume a fixed endowment of a single perfectly divisible good, total W̄ = 200, so x_A + x_B = 200. Utility functions are u_A(x_A) = 2√x_A and u_B(x_B) = √x_B. (a) Derive the equation of the utility possibility frontier (UPF) linking u_A and u_B, and show algebraically that it takes the form of a quarter-ellipse in (u_A, u_B) space. [4 marks] (b) Using the utilitarian social welfare function W = u_A + u_B, find the socially optimal allocation (x_A*, x_B*) on the UPF, and state the resulting utility levels u_A* and u_B*. [5 marks] (c) Repeat part (b) for the Rawlsian social welfare function W = min(u_A, u_B), and explain economically why the Rawlsian optimum allocates the good so differently from the utilitarian optimum, given that agent A converts the good into utility more efficiently than agent B. [5 marks] (d) Attempt to construct two allocations R and S — specifying utility vectors (u_A, u_B) directly, treating utility as a single interpersonally comparable and summable index — such that the move from R to S satisfies a Kaldor-Hicks test based on Σu_i, AND the reverse move from S to R also satisfies that same test. Prove that this is impossible for any two distinct allocations. Then explain, using the distinction between compensating variation and equivalent variation, what property a genuine Kaldor-Hicks test must have for the Scitovsky paradox to actually arise. [6 marks] (e) Explain, with reference to your answers to (a)-(d), why Pareto efficiency alone cannot select a unique point on the UPF, and why the Scitovsky paradox shows that Kaldor-Hicks is not a reliable substitute for a genuine social welfare function. [4 marks]`,
      rubric: `(a) 4 marks: correctly inverts u_A = 2√x_A to get x_A = u_A²/4 (1); correctly substitutes x_B = 200 − x_A into u_B = √x_B to get u_B = √(200 − u_A²/4) (1); correctly rearranges to the standard form u_A²/4 + u_B² = 200 (1); correctly identifies this as a quarter-ellipse and notes d²u_B/du_A² < 0, confirming concavity (1). (b) 5 marks: sets up W(x_A) = 2√x_A + √(200 − x_A) (1); correct first-order condition 1/√x_A = 1/(2√(200 − x_A)) (2); correctly solves to x_A* = 160, x_B* = 40 (1); correctly computes u_A* = 2√160 ≈ 25.3, u_B* = √40 ≈ 6.32 (1). (c) 5 marks: correctly sets the maximin condition 2√x_A = √(200 − x_A) (2); correctly solves to x_A* = 40, x_B* = 160, giving u_A* = u_B* = √160 ≈ 12.65 (2); correctly explains that the Rawlsian planner gives MORE of the good to the less-efficient converter B, the opposite of the utilitarian allocation, precisely to equalise utility levels, since equalising the more-efficient agent's utility requires proportionally less of the good (1). (d) 6 marks: correctly notes that Σu_i(x) is a single real number, so for any two distinct allocations R ≠ S either Σu(S) > Σu(R), Σu(S) < Σu(R), or they are equal — a total order — meaning at most one direction can strictly pass the test, proving no cycle is possible under a scalar-sum criterion (3, allocated as: 1 for setup, 1 for the antisymmetry/total-order argument, 1 for the correct conclusion); correctly defines compensating and equivalent variation as direction-dependent monetary measures and explains that CV and EV need not coincide (or be reciprocal) when the implicit exchange rate between utility and money is not constant across the two reference points, which is exactly what permits both directions to test positive under a genuine KH criterion (3). (e) 4 marks: correctly restates that Pareto efficiency is a partial order — points on the UPF are mutually non-dominated — so it cannot rank distinct efficient points (2); correctly explains, using (d), that a naive Kaldor-Hicks aggregation is prone to path-dependence/cycling and therefore also fails to deliver a reliable complete ranking, motivating a genuine social welfare function with explicit, consistently applied value judgements (2).`,
      exampleTopics: [
        'Deriving the utility possibility frontier algebraically from underlying utility functions and a resource constraint',
        'Constrained optimisation of utilitarian and Rawlsian social welfare functions along the UPF',
        'The Kaldor-Hicks compensation test and the distinction between compensating and equivalent variation',
        'Proving that scalar utility-sum comparisons cannot generate a Scitovsky-style cycle, and identifying what property of true KH tests allows one',
        'Pareto efficiency as a partial order versus a complete social ranking',
      ],
    },
  },
...
The Harsanyi utilitarian theorem (1955) provides a partial rescue. If individuals have von Neumann-Morgenstern utility functions (satisfying the expected utility axioms) and if the social ordering also satisfies those axioms, then the social welfare function must be a weighted sum of individual utilities: W = Σα_i u_i. This gives an axiomatic foundation for utilitarianism without requiring cardinal interpersonal comparisons — the weights α_i can be determined by the social contract. However, the result depends critically on the assumption that society chooses as if it were a single expected-utility maximiser, which is itself a substantive ethical commitment. Rawls explicitly rejected expected-utility reasoning behind the veil of ignorance, arguing that rational agents would choose the maximin rule due to the catastrophic downside of landing at the bottom of the distribution.`,
      ...
      thinkAboutIt: {
        question: `Utilitarianism (W = Σu_i) requires that individual utility be cardinally measurable and unit-comparable across agents — applying a monotonic but non-affine transformation to just one agent's utility index (e.g. v_A = u_A²) can reverse the socially optimal allocation. Does the Rawlsian maximin criterion (W = min u_i) require the same informational strength? Using Sen's distinction between cardinal unit comparability (CUC) and ordinal level comparability (OLC), explain what maximin actually needs and show why maximin can still be manipulated if a transformation is applied to only one agent's index.`,
        answer: `Maximin requires strictly less than utilitarianism: it needs only ordinal level comparability (OLC) — the ability to say, consistently across allocations, whose utility level is lower — not a cardinal scale or unit comparability. This is because maximin only ever asks "who is worst off," a purely ordinal comparison of levels, and never adds or subtracts utility across individuals the way Σu_i does; consequently any transformation applied identically and comparability-preservingly to the whole utility scale leaves the identity of the worst-off agent, and hence the ranking of allocations, unchanged. Utilitarianism by contrast requires cardinal unit comparability: rankings of Σu_i are invariant only to a common positive affine transformation u_i' = a + b·u_i applied to every agent's index with the same a, b, since only affine transformations preserve both the ordering and the relative magnitudes of utility differences that the sum aggregates. However, maximin is not invariant to arbitrary transformations applied to only one agent's index in isolation, because doing so breaks the very interpersonal level-comparison the criterion relies on — for example, replacing u_A with u_A² while leaving u_B unchanged can flip which agent appears worse off at a given allocation, even though nothing about A's own ordinal ranking of bundles has changed. The lesson is that OLC is an assumption about interpersonal comparability of levels that must be applied consistently across agents, not merely about each individual's own preference ordering.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `Two individuals, A and B, share a fixed resource M = 100 (x_A + x_B = 100). Their utility is linear in consumption but the two individuals differ in how efficiently they convert the resource into utility: u_A = 2x_A and u_B = x_B. A social planner ranks allocations using the isoelastic (constant relative inequality-aversion) social welfare function W(ε) = u_A^(1−ε)/(1−ε) + u_B^(1−ε)/(1−ε), for ε ≥ 0, ε ≠ 1. (a) Using the Lagrangian (or direct substitution of the constraint), derive the first-order condition for maximising W(ε) subject to x_A + x_B = 100, and show that the socially optimal share solves to x_A*(ε) = 100 / (1 + 2^(1 − 1/ε)). [7 marks] (b) Evaluate the limit of x_A*(ε) as ε → 0 (the utilitarian case). State which agent receives the entire endowment in this limit, and explain economically why this is optimal for a utilitarian planner given the two agents' different conversion rates. [4 marks] (c) Evaluate x_A*(1) — the log-utilitarian case W = ln u_A + ln u_B — directly by maximising ln(2x_A) + ln(100 − x_A), and confirm your answer agrees with the limit of your general formula from (a) as ε → 1. [3 marks] (d) Evaluate the limit of x_A*(ε) as ε → ∞, and show that it coincides with the Rawlsian maximin allocation obtained directly by solving 2x_A = x_B subject to x_A + x_B = 100. [4 marks] (e) Explain economically why the socially optimal share going to the more "efficient" agent A falls monotonically as ε rises from 0 to ∞, linking your explanation to the interpretation of ε as the coefficient of relative inequality aversion. [4 marks]`,
      rubric: `(a) 7 marks: correctly differentiates each isoelastic term via the constraint substitution to obtain 2(2x_A)^(−ε) for A's marginal social utility and (100 − x_A)^(−ε) for B's (3); correctly sets marginal social utilities equal: 2(2x_A)^(−ε) = (100 − x_A)^(−ε) (1); correctly manipulates logs to isolate (100 − x_A)/(2x_A) = 2^(−1/ε) (2); correctly solves through to x_A*(ε) = 100/(1 + 2^(1−1/ε)) (1). (b) 4 marks: correctly evaluates 1 − 1/ε → −∞ as ε → 0⁺, so 2^(1−1/ε) → 0 and x_A* → 100 (2); correctly explains that with ε = 0 the utilitarian planner maximises the simple sum 2x_A + x_B, and since the marginal social value of giving the good to A (2) permanently exceeds that of giving it to B (1) for all x_A, the corner solution x_A = 100 is optimal (2). (c) 3 marks: correct direct FOC 1/x_A = 1/(100 − x_A) giving x_A*(1) = 50 (2); correctly verifies the general formula gives 100/(1 + 2^0) = 50, matching (1). (d) 4 marks: correctly evaluates 1 − 1/ε → 1 as ε → ∞, so 2^(1−1/ε) → 2 and x_A* → 100/3 ≈ 33.3 (2); correctly solves 2x_A = 100 − x_A directly to get x_A = 100/3, matching (2). (e) 4 marks: correctly identifies that x_A*(ε) is monotonically decreasing in ε, from 100 to 100/3 (1); correctly explains that as ε rises, the planner places increasing weight on the worst-off individual (B, who converts the resource less efficiently) rather than on maximising the aggregate sum, so more of the resource is diverted to B specifically to raise B's own utility level, even though this is "inefficient" in aggregate terms (3).`,
      exampleTopics: [
        'Constrained optimisation of the isoelastic social welfare function via Lagrangian first-order conditions',
        'Taking limits of a parametric social optimum as ε → 0 and ε → ∞ to recover utilitarian and Rawlsian special cases',
        'Economic interpretation of heterogeneous marginal utility "technologies" across agents under differing inequality-aversion parameters',
        'Cross-checking a general parametric formula against direct maximisation in a special case (log utility, ε = 1)',
      ],
    },
  },
...
Arrow-Debreu contingent commodities extend GE theory to uncertainty and time. A contingent commodity is a good defined by its physical characteristics, date of delivery, and the state of the world in which it is delivered. With a complete set of contingent commodity markets, households and firms can trade claims on future contingent consumption, and the Arrow-Debreu competitive equilibrium is Pareto efficient (FWT applies). The model provides the theoretical foundation for derivative pricing and insurance markets. However, real financial markets are incomplete — not all contingent commodities are traded — and in incomplete markets the FWT fails: competitive equilibria are generally constrained-inefficient, which provides a welfare rationale for financial regulation and the provision of social insurance.`,
      ...
      thinkAboutIt: {
        question: `Walras' Law states that the value of aggregate excess demand is identically zero at every price vector, so that if L−1 markets clear, the Lth clears automatically. Does Walras' Law, together with continuity of the excess demand functions, guarantee that a market-clearing price vector actually EXISTS? If not, what additional mathematical result is needed, and what economic condition on excess demand must hold near the boundary of the price simplex for that result to apply?`,
        answer: `No — Walras' Law is an accounting identity that follows mechanically from every agent satisfying their budget constraint with equality; it restricts the RELATIONSHIP between market excess demands at any given price vector but says nothing about whether a price vector exists at which all excess demands are simultaneously zero. Existence requires a genuinely different argument: Arrow and Debreu (1954) proved existence by constructing a price-adjustment mapping from the unit price simplex to itself and applying Kakutani's fixed-point theorem, which guarantees a fixed point provided the mapping is upper hemicontinuous with convex values on a compact, convex domain. For this construction to work, excess demand must also be homogeneous of degree zero in prices (so only relative prices matter, allowing normalisation onto the simplex) and must satisfy a boundary condition — typically that excess demand for any good becomes unbounded, or at least strictly positive, as that good's price approaches zero — which prevents the fixed point from being pushed to a boundary where some good is priced at zero while still being scarce. Walras' Law, homogeneity, continuity, and this boundary condition together are what Arrow-Debreu actually use; Walras' Law alone is necessary bookkeeping, not a sufficient condition for existence.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `Two consumers, A and B, trade two goods X and Y in a pure exchange economy. Endowments: ω_A = (8, 2), ω_B = (2, 8), so the aggregate endowment is (X̄, Ȳ) = (10, 10). Utility functions: u_A(x_A, y_A) = x_A^(1/3)·y_A^(2/3), u_B(x_B, y_B) = x_B^(2/3)·y_B^(1/3). (a) Derive each consumer's Marshallian demand functions for X and Y in terms of prices (p_x, p_y) and their own income. [4 marks] (b) Normalise p_y = 1. Using the market-clearing condition for good X, solve for the equilibrium price p_x*. [6 marks] (c) Compute the full competitive equilibrium allocation (x_A*, y_A*, x_B*, y_B*), and verify explicitly that the market for Y also clears, confirming Walras' Law. [4 marks] (d) Derive the contract curve equation (the locus along which MRS_A = MRS_B), and verify algebraically that your competitive equilibrium allocation from (c) lies on it — as it must, by the First Welfare Theorem. [5 marks] (e) Now suppose endowments are instead ω_A′ = (10, 0) and ω_B′ = (0, 10). Derive a general formula for the equilibrium price p_x as a function of an arbitrary endowment split ω_A = (a, b) [with ω_B = (10−a, 10−b)], evaluate it for both the original endowment and this new one, and explain whether your results show that the competitive equilibrium price ratio is, in general, invariant to how a fixed aggregate endowment is distributed between agents with heterogeneous Cobb-Douglas preferences. [4 marks]`,
      rubric: `(a) 4 marks: correctly applies Cobb-Douglas demand shares to get x_A = (1/3)I_A/p_x, y_A = (2/3)I_A/p_y (2); correctly gets x_B = (2/3)I_B/p_x, y_B = (1/3)I_B/p_y (2). (b) 6 marks: correctly expresses I_A = 8p_x + 2, I_B = 2p_x + 8 (1); correctly writes the market-clearing condition x_A + x_B = 10 (1); correctly simplifies to (8p+2)/(3p) + (4p+16)/(3p) = 10 (2); correctly solves to p_x* = 1 (2). (c) 4 marks: correctly computes x_A* = 10/3, y_A* = 20/3, x_B* = 20/3, y_B* = 10/3 (2); correctly verifies y_A* + y_B* = 10 (2). (d) 5 marks: correctly derives MRS_A = (1/2)(y_A/x_A) and MRS_B = 2(y_B/x_B) (2); correctly sets them equal and expresses the contract curve using feasibility (2); correctly substitutes the equilibrium allocation to verify equality holds, both sides equal to 2 (1). (e) 4 marks: correctly derives the general formula p_x = (20 − b)/(a + 10) (or an equivalent correct expression) by re-solving market clearing with general (a, b) (2); correctly evaluates both cases to get p_x = 1 in each (1); correctly notes that although the price happens to be invariant for these two specific endowment splits, it is not invariant in general — full credit requires recognising this as a special-case coincidence rather than a general result, e.g. by testing a counterexample such as a=0, b=0 giving p_x = 2 (1).`,
      exampleTopics: [
        'Cobb-Douglas Marshallian demand derivation from constrained utility maximisation',
        'Solving market-clearing conditions for equilibrium prices under a chosen numeraire',
        "Verifying Walras' Law numerically by checking that clearing one market implies the other clears",
        'Deriving and verifying the contract curve via MRS equalisation, linking to the First Welfare Theorem',
        'Analysing whether equilibrium prices are invariant to the distribution of a fixed aggregate endowment across heterogeneous consumers',
      ],
    },
  },
...
The theorem\'s implications for democratic theory are profound and contested. Public choice economists (Buchanan, Tullock) interpreted it as showing that collective decisions are inherently arbitrary and that constitutional constraints on majority rule are necessary. Arrow himself drew a more nuanced conclusion: the theorem does not prove that democratic choice is irrational, but that we must either restrict the domain of collective decisions (e.g., to single-peaked preference problems), use cardinal utility comparisons, or accept that some collective choices are weakly rather than strongly ordered. Amartya Sen\'s work on social choice emphasised that partial comparability of utilities — allowing some but not full interpersonal comparisons — can substantially expand the set of satisfactory social choice rules beyond what strict ordinalism permits.`,
      ...
      thinkAboutIt: {
        question: `Arrow's theorem concerns social welfare functions that must produce a complete, transitive social ORDERING over all alternatives for every profile. A social choice FUNCTION need only select a single winner (or winning set) per profile, without ranking the remaining alternatives. Does relaxing this requirement — asking only for a winner, not a full ranking — escape Arrow's impossibility? Explain the relationship to the Gibbard-Satterthwaite theorem.`,
        answer: `Relaxing the requirement to a single-winner selection does weaken the formal scope of Arrow's original theorem, since a choice function is a strictly less demanding object than a complete social ordering, but it does not deliver an escape from impossibility in any meaningful sense — it merely trades one impossibility for a structurally related one. The Gibbard-Satterthwaite theorem shows that any social choice function defined on an unrestricted domain of at least three alternatives that is non-dictatorial and "onto" (every alternative can win for some profile) must be strategically manipulable: there exists some preference profile and some voter who can secure a personally better outcome by misreporting their true preferences. So a rule that avoids Arrow's specific requirements (completeness and transitivity of a full ranking) still cannot avoid either dictatorship or manipulability. Moreover, if we demand that the choice function satisfy minimal consistency properties across varying choice sets — such as Sen's condition α (contraction consistency) and condition β (expansion consistency), needed to rationalise the choice function as coming from an underlying preference relation — Arrow's original axioms and impossibility reassert themselves almost exactly, since a consistent choice function implicitly defines a full ordering via revealed preference over pairs. The single-winner relaxation therefore only postpones the difficulty: it swaps Arrow's dictatorship result for Gibbard-Satterthwaite's manipulability result, and reintroduces Arrow's original result outright once any reasonable consistency requirement is imposed on the choice function.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `(a) Five voters rank three alternatives X, Y, Z as follows — Voter 1: X≻Y≻Z; Voter 2: Y≻Z≻X; Voter 3: Z≻X≻Y; Voter 4: X≻Y≻Z; Voter 5: Y≻Z≻X. Determine the majority-rule outcome of each pairwise contest (X vs Y, Y vs Z, X vs Z), and state whether a Condorcet winner exists. [4 marks] (b) For the same five voters, compute the Borda score of each alternative (2 points for 1st place, 1 for 2nd, 0 for 3rd) and identify the Borda winner. Now suppose a fourth alternative W is introduced, with full rankings: Voter 1: W≻X≻Y≻Z; Voter 2: Y≻Z≻W≻X; Voter 3: Z≻W≻X≻Y; Voter 4: W≻X≻Y≻Z; Voter 5: Y≻Z≻W≻X (note these reduce to the profile in (a) once W is removed). Recompute Borda scores (3/2/1/0 for 1st-4th) for X, Y, Z, W, and show that the relative ranking of X and Z has flipped purely due to the introduction of W — demonstrating a violation of Independence of Irrelevant Alternatives. [6 marks] (c) Define a voting rule f on the same five voters: for any two alternatives p and q, society ranks p above q if and only if both Voter 1 and Voter 2 strictly prefer p to q; if Voter 1 and Voter 2 disagree, society is indifferent between p and q under f. (i) Show that the coalition {Voter 1, Voter 2} is decisive for every pair of alternatives under f. (ii) Show that neither Voter 1 nor Voter 2 is individually decisive for every pair — i.e. f is not a dictatorship of either voter alone. (iii) Explain why f's failure to be a dictatorship does not contradict Arrow's theorem, identifying which of Arrow's requirements f fails to satisfy in full generality. [5 marks] (d) Five voters have single-peaked preferences over a one-dimensional policy space (utility declines with Euclidean distance from an ideal point), with ideal points at 2, 4, 5, 7, and 10. Identify the median voter's ideal point, and prove it is a Condorcet winner by explicitly comparing it in pairwise majority votes against two illustrative alternative proposals of your choice. State which of Arrow's axioms is relaxed to obtain this positive result, and why. [6 marks]`,
      rubric: `(a) 4 marks: correct pairwise tallies — X beats Y 3-2 (1); Y beats Z 4-1 (1); Z beats X 3-2 (1); correctly concludes the pairwise relation cycles (X beats Y, Y beats Z, Z beats X) so no Condorcet winner exists (1). (b) 6 marks: correct 3-alternative Borda scores X=5, Y=6, Z=4, correctly identifying Y as the Borda winner (2); correctly computes the 4-alternative Borda scores from the given profile: X=5, Y=8, Z=7, W=10 (2); correctly observes that X and Z have swapped relative order (X>Z becomes Z>X) purely due to W's introduction, and correctly states this demonstrates a violation of IIA, since the social ranking of X vs Z should depend only on individual X-vs-Z comparisons, unchanged by adding W (2). (c) 5 marks: correctly shows {Voter 1, Voter 2} is decisive by direct application of f's definition (1); correctly shows neither voter alone is decisive — a lone preference is overridden whenever the other disagrees, triggering the indifference clause rather than a determinate ranking (2); correctly explains that f fails to always generate a complete, strict social ordering for every profile (it produces indifference whenever Voters 1 and 2 disagree, regardless of Voters 3-5's unanimity), so f does not satisfy the full Unrestricted-Domain/completeness requirement Arrow's contraction argument relies on — hence the absence of a single dictator here does not contradict Arrow's theorem (2). (d) 6 marks: correctly identifies the median ideal point as 5 (Voter 3) (1); correctly computes distances/preferences for a comparison against proposal 6, showing 5 wins 3-2 (2); correctly computes distances/preferences for a comparison against proposal 2, showing 5 wins 4-1 (2); correctly states that Unrestricted Domain is the axiom relaxed — restricting to single-peaked preferences rather than allowing arbitrary preferences — which by Black's median voter theorem guarantees transitive majority rule and a Condorcet winner (1).`,
      exampleTopics: [
        'Pairwise majority comparison to identify Condorcet cycles',
        'Borda count computation and constructing a numerical IIA violation by varying the position of a decoy alternative across voters',
        'Formal definition and identification of decisive coalitions, and distinguishing coalition decisiveness from individual dictatorship',
        "Median voter theorem: identifying the Condorcet winner under single-peaked preferences and relating it to relaxing Arrow's Unrestricted Domain axiom",
      ],
    },
  },
...
The design of kidney exchange illustrates the power of market design beyond traditional price mechanisms. Direct kidney exchange between incompatible donor-recipient pairs is infeasible (wrong blood types), but chains of compatible swaps — a "non-simultaneous extended altruistic donor chain" — can vastly expand the pool of potential matches. Roth, Sönmez, and Ünver applied matching theory to design exchange programmes that find efficiency-maximising chains while respecting ethical constraints (no direct payment for organs — monetising kidneys is banned in most jurisdictions, illustrating how market design must respect "repugnance" constraints). The resulting transplant chain programmes have saved thousands of lives in the US, demonstrating that welfare economics and market design can address problems where standard price mechanisms are both legally prohibited and practically inadequate.`,
      ...
      thinkAboutIt: {
        question: `The Coase theorem claims that, under zero transaction costs, private bargaining reaches the same EFFICIENT QUANTITY of the externality-generating activity regardless of which party holds the property right — only the distribution of wealth differs. This invariance claim implicitly assumes there are no wealth effects. If the polluter's and the victim's marginal utility of income differ (i.e. utility is not quasi-linear in money), explain why the negotiated quantity of pollution can actually depend on the initial assignment of rights, even with zero transaction costs.`,
        answer: `The initial assignment of the property right is itself a lump-sum wealth transfer to whichever party receives it, and if utility is not quasi-linear in money, a change in wealth shifts that party's marginal rate of substitution between money and the externality — that is, it changes how much of the good they are willing to trade at any given price. When the polluter holds the right, the victim must buy abatement using their own, relatively lower, wealth, and their willingness-to-pay is capped by their income and the marginal utility of money at that income level; when the victim holds the right, the polluter must buy permission to pollute, and the compensation the victim now demands (willingness-to-accept) is evaluated at the victim's higher wealth level, typically producing a different reservation price because marginal utility of income has fallen. This is exactly the source of the well-documented empirical gap between willingness-to-pay and willingness-to-accept: the two measures coincide only in the absence of income effects. Consequently, the negotiated equilibrium quantity Q* — where the marginal benefit of one more unit of the activity to its holder equals the marginal cost imposed on the other party — can differ across the two rights regimes, because the marginal cost/benefit schedules themselves are wealth-dependent. The Coase theorem's strong quantity-invariance result therefore holds exactly only in the special case of quasi-linear utility (constant marginal utility of the numeraire), which is why it is usually stated as an idealisation; with income effects present, only the broader claim that bargaining reaches some point on the (now wealth-dependent) Pareto frontier survives, not that it reaches the identical quantity regardless of rights.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `A chemical plant's private marginal cost of production is PMC(Q) = 15 + 3Q, and it sells output at the fixed market price P = 75. Production also imposes an external marginal cost on a nearby town of EMC(Q) = Q. (a) Find the private (unregulated) equilibrium output Q_p, where the firm sets P = PMC(Q). [2 marks] (b) Derive the social marginal cost SMC(Q) = PMC(Q) + EMC(Q), and find the socially optimal output Q*, where P = SMC(Q). [4 marks] (c) Derive the optimal Pigouvian tax t* = EMC(Q*), and verify algebraically that a profit-maximising firm facing PMC(Q) + t* chooses exactly Q*. [4 marks] (d) Calculate the deadweight loss from producing at Q_p instead of Q* (state the relevant triangle and compute its area). [4 marks] (e) Now suppose instead the town is granted the legal right to zero pollution, so the plant must bargain with the town to purchase permission to produce. Total net social surplus from producing any quantity Q (relative to Q = 0) is ∫₀^Q [P − SMC(x)] dx. Compute the maximum total surplus available from bargaining up to Q* (relative to the zero-pollution status quo), and state the condition on the town's total transaction cost T under which the Coase theorem still predicts the parties reach Q* through bargaining. [4 marks] (f) Now suppose there are two chemical plants instead of one, with different marginal abatement cost (MAC) curves: MAC₁(q₁) = 2q₁ and MAC₂(q₂) = q₂, where q₁ and q₂ denote tonnes of pollution abated by each plant. A regulator imposes a cap requiring total abatement q₁ + q₂ = 30 tonnes. Find the cost-effective allocation of abatement (q₁*, q₂*) that minimises total abatement cost subject to the cap, and state the resulting equilibrium permit price. [5 marks]`,
      rubric: `(a) 2 marks: correctly sets 75 = 15 + 3Q and solves Q_p = 20. (b) 4 marks: correctly derives SMC(Q) = 15 + 4Q (2); correctly sets 75 = 15 + 4Q and solves Q* = 15 (2). (c) 4 marks: correctly states t* = EMC(15) = 15 (2); correctly verifies 75 = 15 + 3Q + 15 gives Q = 15 = Q* (2). (d) 4 marks: correctly identifies the DWL triangle bounded by SMC and P between Q* and Q_p (2); correctly computes DWL = ½ × (20 − 15) × (SMC(20) − 75) = ½ × 5 × 20 = 50, accepting the equivalent computation via SMC(20) − PMC(20) = EMC(20) = 20 (2). (e) 4 marks: correctly sets up the integral ∫₀^15 (75 − (15 + 4x)) dx = ∫₀^15 (60 − 4x) dx (2); correctly evaluates to [60x − 2x²] from 0 to 15 = 900 − 450 = £450 (1); correctly states the Coasean bargaining condition T < 450 — the transaction cost must be less than the total available surplus for a mutually beneficial deal to be struck (1). (f) 5 marks: correctly sets up the cost-effectiveness condition MAC₁(q₁*) = MAC₂(q₂*) (2); correctly solves 2q₁ = 30 − q₁ (using q₂ = 30 − q₁) to get q₁* = 10, q₂* = 20 (2); correctly states the equilibrium permit price = MAC₁(10) = MAC₂(20) = 20 (1).`,
      exampleTopics: [
        'Deriving social marginal cost by adding external marginal cost to private marginal cost, and solving for the socially optimal output',
        'Computing the Pigouvian tax and verifying it realigns private incentives with the social optimum',
        'Deadweight loss triangle calculation from uninternalised externalities',
        'Applying the Coase theorem\'s "gains from trade must exceed transaction costs" logic under an alternative property-rights assignment',
        'Cost-effective allocation of abatement across heterogeneous firms via equalisation of marginal abatement costs under a cap',
      ],
    },
  },
]
