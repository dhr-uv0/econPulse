...
The dual-process framework has also reshaped the design of financial regulation. The EU's MiFID II directive and the U.S. SEC's Regulation Best Interest both mandate simplified disclosure documents, recognizing that complex prospectuses engage only System 2 while most retail investors make decisions with System 1. Experimental work by Shlomo Benartzi and Thaler on retirement savings shows that employees defaulted into a "Save More Tomorrow" program (which gradually escalated contribution rates) increased their savings rate by an average of 3.5 percentage points over 28 months — because the program exploited System 1's status quo bias rather than fighting it. This approach — changing the environment rather than trying to improve cognition — is the foundational insight of nudge theory explored in Lesson 3.`,
      ...
      thinkAboutIt: {
        question: `Gerd Gigerenzer's "recognition heuristic" — when comparing two cities by population, choose the one you recognize and ignore the one you don't — sometimes outperforms a full-information multiple regression model in out-of-sample prediction, a phenomenon called the "less-is-more" effect. Does this contradict the standard behavioral-economics claim that heuristics are inferior shortcuts relative to fully rational computation? Use the concept of ecological rationality to explain.`,
        answer: `It does not contradict the standard claim, but it does sharply qualify it. A heuristic is not intrinsically good or bad — its performance depends on the fit between its structure and the statistical structure of the environment (Gigerenzer's "ecological rationality"). Recognition is a valid cue only when it correlates with the criterion being judged (larger, more prominent cities are recognized more often), and in that setting a one-variable rule can actually generate lower out-of-sample error than a regression with many parameters, because the regression estimates additional coefficients from noisy sample data and overfits — a textbook bias-variance trade-off in which the simpler model has higher bias but much lower variance. This is precisely why the effect is called "less-is-more": less information and less computation can reduce total prediction error when the additional information mostly adds estimation noise rather than genuine signal. Crucially, the recognition heuristic fails, and can be badly biased, in environments where recognition does not track the criterion (e.g., recognizing cities because of unrelated media coverage of a scandal rather than size), so its success is environment-specific, not universal. The olympiad-level takeaway is that "boundedly rational" does not mean "worse than optimal computation in every environment" — it means decision rules are adapted to particular information structures, and a heuristic can be normatively superior to a fully rational model precisely because the fully rational model's data demands exceed what the environment can reliably supply.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `A national pension regulator observes that when workers are shown an example contribution rate of "3%" in enrollment materials, average worker-selected contribution rates cluster tightly around 3-4%, even though financial planners recommend 12-15% for adequate retirement income. (a) Identify the heuristic responsible for this clustering and explain the underlying System 1 / System 2 mechanism using Kahneman's dual-process model. [4 marks] (b) The regulator considers three redesigns: (i) removing the numeric example entirely; (ii) changing the example to "12%"; (iii) presenting three example rates (6%, 12%, 18%) and asking workers to select or specify their own. Predict, with reasoning, which redesign is likely to produce the highest average contribution rate, and explain the mechanism using anchoring-and-adjustment theory. [6 marks] (c) A colleague objects that redesign (ii) is manipulative because it exploits a cognitive bias to steer behavior toward the regulator's preferred outcome. Using the distinction between bounded rationality and outright irrationality, construct an argument for why anchoring on 12% could still be defensible rather than manipulative. [5 marks] (d) Explain why representativeness, rather than anchoring, might instead be the dominant mechanism if workers judge their own retirement adequacy by comparing themselves to peers who are also under-saving, and describe one empirical test that could distinguish anchoring from representativeness as the operative bias in this scenario. [5 marks]`,
      rubric: `(a) 4 marks: correctly names anchoring bias / insufficient adjustment (1); explains that the example number functions as an anchor processed by System 1 (1); explains that adjustment away from the anchor is effortful System 2 work that is rarely performed fully (1); explicitly links the small gap between the anchor (3%) and chosen rates (3-4%) to "insufficient adjustment" (1). (b) 6 marks: correct prediction that (iii), or at minimum (ii), raises average rates furthest above the (i) baseline, with reasoning (2); explains that removing the anchor (i) does not guarantee higher rates because workers still need a reference point and may default to very low or zero contributions absent guidance (2); explains that presenting multiple anchors (iii) widens the plausible range and shifts the effective anchor upward while preserving perceived choice — a stronger nudge than a single higher anchor because it avoids anchoring workers rigidly on one number (2). (c) 5 marks: correctly distinguishes bounded rationality (constrained but purposeful choice) from irrationality (1); argues that since some anchor is unavoidable in any choice architecture (there is no "neutral" default), selecting the anchor that aligns with expert-recommended savings rates satisfies the libertarian-paternalism criterion of improving outcomes by the workers' own reflective standards (2); notes the anchor does not restrict any option — workers remain fully free to select below 12% (2). (d) 5 marks: explains representativeness as judging one's own adequacy by similarity to a socially "typical" (but actually under-saving) peer group rather than by an objective adequacy benchmark (2); correctly notes this predicts persistent under-saving even without any numeric anchor being shown (1); proposes a valid empirical test, e.g. a between-subjects experiment varying only the anchor (holding peer-comparison information constant) versus varying only peer-comparison information (holding the anchor constant), to isolate which manipulation shifts behavior (2).`,
      exampleTopics: [
        'Anchoring-and-insufficient-adjustment and how the size of an anchor propagates into final judgments',
        "Kahneman's dual-process (System 1 / System 2) architecture and the cost of System 2 engagement",
        'Representativeness heuristic and base-rate neglect in self-assessment against a peer reference group',
        "Thaler and Sunstein's libertarian-paternalism welfare criterion as a test for legitimate choice-architecture design",
        'Experimental design for isolating one behavioral mechanism from a confounded alternative explanation',
      ],
    },
  },
...
Prospect theory also generates precise predictions about insurance demand that standard theory struggles to explain. Expected utility theory predicts insurance demand only against large, potentially ruinous losses — yet empirical surveys consistently find high demand for low-deductible policies that cover small, easily absorbable losses (appliance warranties, cell phone insurance). The probability weighting function explains this: small loss probabilities are overweighted, making the insurance seem more valuable than the actuarially fair premium. Simultaneously, the loss aversion in the value function amplifies the psychological impact of even small losses, reinforcing insurance demand. Policy design can exploit or correct these tendencies: requiring automatic enrollment in catastrophic health plans (overcoming status quo bias) while banning extended warranties on low-cost goods (correcting probability overweighting) are both defensible applications of prospect theory insights.`,
      ...
      thinkAboutIt: {
        question: `Matthew Rabin's (2000) calibration theorem shows that an expected-utility maximizer who, at every wealth level, rejects a 50/50 gamble of losing $100 or gaining $110 (a mild small-stakes risk aversion) must — if that rejection is explained purely by concavity of a utility-of-wealth function — also reject a 50/50 gamble of losing $1,000 for any finite possible gain, no matter how large. Since virtually no one would reject such an absurdly favorable large-stakes gamble, what does this imply about "diminishing marginal utility of wealth" as an explanation of everyday small-stakes risk aversion, and which specific feature of prospect theory replaces it?`,
        answer: `Rabin's result is a proof by calibration: because a smooth, increasing, concave utility function's curvature cannot change fast enough over a small wealth range and then vanish over a large one, rejecting a modest gamble at all wealth levels forces the marginal utility of wealth to fall so steeply that the implied risk aversion over large stakes becomes economically absurd — the theorem shows that no coherent concave utility-of-wealth function can generate mild aversion to the small gamble without also generating near-total unwillingness to accept even enormously favorable large gambles. This means diminishing marginal utility of wealth cannot be the true explanation for observed small-stakes risk aversion, since real decision-makers plainly do accept large favorable gambles. Prospect theory resolves the paradox by relocating the source of small-stakes risk aversion away from the curvature of a wealth-level utility function and into loss aversion at the reference point: the kink in the value function, where losses are weighted roughly 2 to 2.5 times as heavily as equal-sized gains, produces aversion to a $100/$110 coin flip without saying anything about the curvature of value further out in the gains or losses domains. Because loss aversion is a local, reference-dependent property rather than a global property of a wealth function, it can generate realistic small-stakes aversion while remaining fully compatible with willingness to accept large favorable gambles, since the diminishing-sensitivity curvature away from the reference point is comparatively mild (the estimated exponents ≈0.88 are only slightly concave/convex). This is why prospect theory, not expected utility with concave utility of wealth, is now the standard explanation economists give for modest-stakes insurance-like behavior.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `Use the Tversky and Kahneman (1992) cumulative prospect theory value function v(x) = x^0.88 for x ≥ 0 and v(x) = −2.25(−x)^0.88 for x < 0, with the reference point at current wealth, and ignore probability weighting throughout (assume decision weights equal objective probabilities). (a) Compute v($1,000) and v(−$1,000) to one decimal place, verify that |v(−1000)| exceeds v(1000), and express the ratio as the empirical loss-aversion coefficient. [3 marks] (b) A gamble offers a 50% chance of gaining $1,000 and a 50% chance of losing $1,000 relative to current wealth. Compute the prospect value of this gamble and state whether a prospect-theory agent accepts it. [4 marks] (c) Find, to the nearest dollar, the minimum gain G such that the agent is indifferent between the status quo (value 0) and a 50/50 gamble of gaining G or losing $1,000. [5 marks] (d) A trader has just lost $2,000 relative to their original reference point and is now deciding whether to accept a new 50/50 gamble with final outcomes of −$1,000 or −$3,000 relative to the original reference point, versus stopping and accepting the certain −$2,000. Compute v(−1000), v(−2000), and v(−3000), determine whether the trader prefers to gamble or to stop, and explain the result using "diminishing sensitivity" in the loss domain. [5 marks] (e) Relate the result in (d) to the "break-even effect" documented by Thaler and Johnson (1990) and to the disposition effect discussed in the lesson, and explain one risk-management implication for whether traders who have just suffered large losses should retain full trading discretion. [3 marks]`,
      rubric: `(a) 3 marks: correct v(1000) ≈ 436.9 (1); correct v(−1000) ≈ −983.0 (1); correctly identifies the ratio |v(−1000)|/v(1000) = 2.25 as the loss-aversion coefficient λ (1). (b) 4 marks: correctly computes 0.5×436.9 + 0.5×(−983.0) ≈ −273.1 (2); correctly concludes the prospect value is negative so the gamble is rejected (1); correctly notes this is despite the gamble's zero expected monetary value, illustrating loss aversion rather than risk aversion over money per se (1). (c) 5 marks: sets up 0.5×v(G) + 0.5×v(−1000) = 0 (1); correctly isolates v(G) = 983.0 (1); correctly inverts G = 983.0^(1/0.88) (1); correct numerical answer G ≈ $2,520 (1); notes this exceeds $1,000 by roughly the loss-aversion multiple, consistent with part (a) (1). (d) 5 marks: correct v(−1000) ≈ −983.0, v(−3000) ≈ −2,581.4, v(−2000) ≈ −1,806.5 (2, partial credit for one correct); correctly computes expected prospect value of gambling as 0.5×(−983.0) + 0.5×(−2,581.4) ≈ −1,782.2 and compares to −1,806.5 for stopping, correctly concluding the trader prefers to gamble since −1,782.2 > −1,806.5 (2); correctly explains that convexity/diminishing sensitivity in the loss domain means the marginal pain of moving from −2,000 to −3,000 is smaller than the marginal pain of the initial loss, so the downside of gambling further is muted while the chance of a partial recovery is valued highly (1). (e) 3 marks: correctly names the break-even effect / "get-evenitis" (1); links it to the disposition effect's asymmetric holding of losers (1); draws a defensible risk-management implication, e.g. mandatory cooling-off periods or position-size limits imposed algorithmically rather than left to trader discretion after large losses (1).`,
      exampleTopics: [
        'Applying the Tversky-Kahneman power value function numerically to gains and losses',
        'Loss aversion coefficient λ and its role in determining gamble acceptance/rejection',
        'Reference-point adaptation after a prior loss and its effect on subsequent risk-taking',
        'Diminishing sensitivity (convexity in losses, concavity in gains) as distinct from loss aversion',
        'Break-even effect and disposition effect as applied consequences of the value function',
      ],
    },
  },
...
Nudge theory faces serious theoretical and empirical challenges that olympiad candidates must understand. First, the normative benchmark is contested: nudges are supposed to align behavior with individuals' own considered preferences, but identifying those preferences is non-trivial when preferences are inconsistent or context-dependent. If a person's "true preference" is revealed only in one context (calm, deliberate choice), which context is canonical? Second, nudges can be deployed by private actors with interests opposed to the chooser — a problem Sunstein's critics call "dark nudging" or "sludge" (friction deliberately added to make beneficial choices harder, such as complex opt-out processes for recurring subscriptions). Third, experimental evidence on nudge persistence is mixed: many nudges fade as salience decreases, so without reinforcing architecture they may produce only short-run behavioral change. These critiques inform a rich academic debate between nudge advocates and those who prefer "boosts" — interventions that enhance decision-making competence rather than exploit System 1 biases.`,
      ...
      thinkAboutIt: {
        question: `Schultz et al. (2007) sent households a card comparing their electricity use to the neighborhood average (a descriptive social norm). Above-average users reduced consumption as intended — but below-average users significantly increased consumption afterward, a "boomerang effect." Explain the psychological mechanism that causes a purely informational nudge to backfire, and describe the design fix that Schultz and colleagues found eliminates it without sacrificing the reduction among high users.`,
        answer: `A descriptive norm communicates only what is typical, not what is approved of, and System 1 tends to read "typical" as tacit permission to move toward the average — for below-average users, learning that they consume less than their neighbors implicitly signals that they have "room" to consume more without deviating from the local norm, so the information itself creates a new, unintended reference point that pulls their behavior upward through mean-reversion via social comparison rather than downward through conservation motivation. This is a direct illustration of why choice architecture is not simply "more accurate information is always better": a nudge that changes only descriptive content, with no evaluative valence attached, can move behavior in the direction of the norm regardless of whether that direction is desirable for a given recipient. The fix Schultz et al. validated is to append an injunctive-norm cue — a smiling emoticon for below-average households and a frowning emoticon for above-average households — which supplies the missing approval/disapproval signal that the raw descriptive comparison lacked. With the emoticon added, above-average users still reduced consumption (reinforced by social disapproval) while below-average users no longer increased consumption, because the smiley communicated that their behavior was already socially sanctioned rather than merely typical. The general design principle is that any social-norm nudge based purely on descriptive statistics risks a boomerang effect for recipients on the "good" side of the norm, and should be paired with an injunctive (evaluative) signal to lock in the desired direction for both groups.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `A Ministry of Health wants to raise HPV vaccination uptake among adolescents from a national baseline of 45%. (a) Using the three-way distinction between defaults, framing, and salience nudges, design one intervention of each type applicable to a national vaccination scheduling system, and explain the psychological mechanism through which each is predicted to work. [6 marks] (b) A junior policy analyst proposes sending parents a letter stating "9 out of 10 parents in your area have already vaccinated their child" (a descriptive social-norm nudge). Drawing on the boomerang-effect research discussed in the lesson, explain the specific risk this intervention carries in districts where actual local vaccination rates are well below 90%, and propose a concrete design fix. [5 marks] (c) Apply Thaler and Sunstein's normative criterion for legitimate nudging to evaluate whether switching the default from opt-in to opt-out vaccination scheduling (with a signed refusal form required to decline) is ethically defensible, explicitly addressing the objection that vaccination, unlike retirement savings, involves an irreversible bodily intervention. [5 marks] (d) After implementation, opt-out scheduling raises vaccination rates from 45% to 78%, but a follow-up evaluation finds that 15 percentage points of the increase reverses within two years because parents who never experienced an active decision moment report significantly lower knowledge of contraindications and side effects. Using the concept of "sludge" and the distinction between nudge persistence and nudge fade-out, propose a redesign that preserves most of the effectiveness gain while addressing the informed-consent shortfall. [4 marks]`,
      rubric: `(a) 6 marks (2 per intervention): default example (e.g., automatically pre-booking a vaccination slot at the routine adolescent check-up, requiring active cancellation to opt out) with correct mechanism (status quo bias / effort cost of deviating) (2); framing example (e.g., messaging emphasizing "protects against cancer" as a gain-frame or "unprotected children face a X% risk" as a loss-frame, consistent with prospect-theory-driven framing effects) with correct mechanism (2); salience example (e.g., a reminder text sent the week of eligibility, or highlighting the vaccine at the top of a digital health portal) with correct mechanism (limited attention / availability) (2). (b) 5 marks: correctly identifies that below-90%-rate districts risk a boomerang effect, where parents below the stated norm may reduce urgency because they infer their behavior is not unusual (2); correctly cites the descriptive-vs-injunctive norm distinction from Schultz et al. (1); proposes a valid fix, e.g. adding an injunctive/evaluative cue, or tailoring the message only to areas where local uptake genuinely exceeds a high threshold rather than using one national statistic everywhere (2). (c) 5 marks: correctly states the Thaler-Sunstein criterion (nudge legitimate if it aligns with what the chooser would want on reflection, and preserves free choice) (2); constructs a coherent argument addressing irreversibility — e.g., that irreversibility raises the bar for legitimacy since a wrongly-nudged choice cannot be corrected later, requiring stronger safeguards (an easy, genuinely low-friction opt-out and full information provided regardless of default) (2); does not conflate opt-out defaults with coercion, correctly noting refusal remains fully available (1). (d) 4 marks: correctly identifies the reduced informed-consent outcome as a sludge-adjacent cost of removing the active decision moment, not classic profit-motivated sludge (1); proposes a defensible redesign, e.g. keeping the opt-out default but requiring parents to view or acknowledge a short mandatory information module before the appointment is finalized, preserving low friction while restoring an information checkpoint (2); explains why this addresses fade-out (knowledge gap) without reverting to a high-friction opt-in default that would sacrifice most of the participation gain (1).`,
      exampleTopics: [
        'Distinguishing default, framing, and salience nudges and their distinct psychological mechanisms',
        'Descriptive versus injunctive social norms and the conditions under which norm messaging boomerangs',
        "Thaler and Sunstein's welfare criterion for legitimate libertarian paternalism",
        'Sludge as friction that undermines a nudge\'s benefit, including unintended informational costs',
        'Designing choice architecture that balances effectiveness against informed consent and reversibility',
      ],
    },
  },
...
The RCT methodology itself has generated controversy within development economics. Angus Deaton (2015 Nobel laureate) and Nancy Cartwright have argued that RCTs in development economics suffer from external validity problems: results from an experiment in one village in Kenya may not generalize to villages in Tanzania, let alone to policy at national scale, because the context (institutions, prices, social norms) differs in ways that interaction terms with the treatment cannot capture in a single experiment. Proponents of RCTs respond that all empirical methods face external validity challenges, and that RCTs at least ensure internal validity — the treatment effect in the study population is identified without bias, which is a prerequisite for any generalization. The methodological debate has shifted toward "systematic reviews" and meta-analyses that pool evidence across multiple RCTs to identify more robust and generalizable findings — a sign of the field's maturation.`,
      ...
      thinkAboutIt: {
        question: `Standard poverty-trap models — the S-curve savings/nutrition trap and the Murphy-Shleifer-Vishny coordination-failure model — both assume agents optimize rationally given their constraints; the trap lies in the feasible incentive structure, not in decision-making itself. Mullainathan and Shafir's "scarcity" research argues that poverty imposes a cognitive "bandwidth tax" that measurably impairs attention and working memory, creating a genuinely behavioral poverty trap. How does this mechanism differ structurally from the S-curve and Big Push models, and why does it imply that even a one-time transfer sufficient to cross a capital threshold might fail to permanently lift a household out of poverty?`,
        answer: `The S-curve and Big Push models locate the trap in the environment facing a fully rational optimizer: below a capital threshold, feasible returns to saving are too low to escape; in the coordination-failure case, no individual firm can profitably deviate unilaterally. In both, correctly relaxing the external constraint — enough capital, or enough coordinated investment — is sufficient in principle for the rational agent to thereafter optimize its way to the high equilibrium. Mullainathan and Shafir's mechanism is structurally different: scarcity itself consumes a scarce cognitive resource (attention and working memory, or "bandwidth"), so a person preoccupied with an immediate shortfall has measurably less cognitive capacity left for planning, self-control, and evaluating trade-offs — Mani et al. (2013) found Indian sugarcane farmers scored roughly 9-10 IQ-equivalent points lower on cognitive tests before harvest (when poor) than after harvest (when relatively flush), a within-person effect comparable to losing a full night's sleep. This means the trap operates through the agent's decision-making capacity itself, not merely through the external opportunity set — so a household could receive a capital transfer large enough to cross the S-curve threshold and still make poor investment, repayment, or health decisions if other sources of scarcity (debt stress, food insecurity, time poverty) continue to tax bandwidth after the transfer arrives. This explains why "graduation" programs that pair asset transfers with intensive coaching and structured savings support have consistently outperformed equally-sized pure cash transfers in RCT evidence: relieving the capital constraint and relieving the cognitive constraint are not the same intervention, and both may need to be addressed jointly for an escape to be durable.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `A stylized economy's income dynamics are given by y_(t+1) = f(y_t), where households use a traditional technology when income is at or below the adoption threshold and a modern technology once income exceeds it: f(y) = 0.5y for 0 ≤ y ≤ 25 (traditional technology), and f(y) = 0.8y + 20 for y > 25 (modern technology, accessible only once income exceeds the threshold). (a) Find the steady state within the traditional-technology regime and the steady state within the modern-technology regime. For each, determine whether it is stable by comparing the regime's slope to 1 and by reasoning about whether nearby trajectories converge toward or diverge from it. [6 marks] (b) A household starts at y₀ = 30 (already above the threshold). Compute y₁, y₂, and y₃, and state what income level the household converges to in the long run. [4 marks] (c) A second household starts at y₀ = 20 (in the trap). Determine the exact minimum one-time cash transfer required so that, after the transfer, the household's income for next period is in the modern-technology regime, and explain precisely why a transfer of exactly this minimum size is not sufficient on its own (i.e., why the transfer must exceed, not merely equal, the gap to the threshold). [4 marks] (d) Design a three-arm randomized evaluation to test whether a large one-time transfer (of the size found in (c) or larger) produces better long-run income outcomes than a smaller recurring monthly transfer that never individually crosses the threshold, against an untreated control. State the outcome variable, the minimum follow-up horizon needed to distinguish a genuine trap-escape effect from a temporary consumption boost, and one specific threat to internal validity that applies especially to the one-time-transfer arm. [6 marks]`,
      rubric: `(a) 6 marks: correctly finds the traditional-regime fixed point at y = 0 (1) and correctly argues it is stable/attracting because f(y) = 0.5y < y for all y > 0 in that regime, so trajectories starting below 25 shrink toward 0 (2); correctly finds the modern-regime fixed point by solving 0.8y + 20 = y → y = 100 (1) and correctly argues it is stable because the regime's slope 0.8 < 1, so deviations shrink over time (2). (b) 4 marks: correctly computes y₁ = 0.8(30)+20 = 44 (1), y₂ = 0.8(44)+20 = 55.2 (1), y₃ = 0.8(55.2)+20 = 64.16 (1), and correctly states the sequence converges monotonically to 100 (1). (c) 4 marks: correctly identifies that next-period income must place the household in the regime y > 25, requiring current income (after transfer) to exceed 25 (1); correctly computes the gap from y₀ = 20 to the threshold as 5 (1); correctly explains that a transfer of exactly 5 leaves the household at exactly y = 25, which is defined as still in (or on the boundary of) the traditional regime and therefore still subject to y_(t+1) = 0.5y = 12.5, relapsing toward 0 (2, allow full credit for equivalent reasoning that the transfer must be strictly greater than 5). (d) 6 marks: correct outcome variable, e.g. household income or asset value measured several years post-transfer (1); correct minimum horizon reasoning — must be long enough for the modern-technology regime's convergence dynamics to visibly separate the arms from a one-off consumption bump, e.g. at least 3-5 periods given the 0.8 convergence rate found in (a)-(b) (2); valid internal-validity threat specific to the lump-sum arm, e.g. general-equilibrium spillovers in which a large local injection of cash raises local prices or wages, affecting the control group's measured outcomes (violating SUTVA) — or anticipation/behavioral effects around the timing of a known large payment (3, any one well-explained threat with correct reasoning).`,
      exampleTopics: [
        'Finding and classifying fixed points (steady states) of a piecewise transition function by comparing slope to 1',
        'Iterating a difference equation to trace a trajectory toward a stable equilibrium',
        'Big Push logic: why a transfer must strictly exceed the threshold gap, not merely equal it',
        'Randomized evaluation design: arms, outcome variables, and follow-up horizon appropriate to the hypothesis',
        'Threats to internal validity in field experiments, including spillovers/SUTVA violations',
      ],
    },
  },
...
The growth accounting literature has illuminated the "East Asian miracle" debate. Young (1995) and Collins and Bosworth (1996) decomposed East Asian growth and found that the majority was explained by factor accumulation (rising capital-output ratios, increased education and participation rates) rather than TFP growth — challenging the narrative that East Asia's growth was driven by superior technology adoption. Paul Krugman famously used these results to argue that East Asian growth was "perspiration, not inspiration" — unsustainable in the long run because factor accumulation faces diminishing returns, unlike TFP-driven growth. However, more recent estimates using better price indices and correcting for composition effects find larger TFP contributions in some East Asian economies, and the debate remains methodologically live. The key olympiad insight is that TFP accounting is sensitive to assumptions about factor shares (α), the quality of capital and labor measures, and the treatment of human capital — so point estimates should be interpreted with appropriate uncertainty.`,
      ...
      thinkAboutIt: {
        question: `A researcher regresses each country's 1960-2000 growth rate on its 1960 log GDP per capita across a panel of 100 structurally similar countries, finds a negative and statistically significant coefficient β = −0.02, and concludes: "conditional convergence is occurring — the world income distribution is becoming more equal among these countries." Identify the statistical flaw in this inference, sometimes called "Galton's fallacy," and explain what additional evidence is required to support the stronger claim about dispersion.`,
        answer: `The flaw is conflating β-convergence (a negative coefficient in a regression of growth on initial income) with σ-convergence (a falling cross-sectional variance, or standard deviation, of log income over time) — the two are logically distinct and the first does not imply the second. Francis Galton observed the same statistical pattern regressing children's heights on parents' heights: tall parents tend to have somewhat shorter children and short parents somewhat taller children (a negative coefficient, "regression to the mean"), yet the population-wide variance of adult height has not been shrinking toward zero across generations — the negative coefficient is partly a statistical artifact of imperfect correlation between two noisy measurements, not proof of literal convergence in the distribution. Applied to growth: pure mean reversion driven by transitory shocks or measurement error in initial income can generate a negative β even while the cross-sectional dispersion of income (σ) stays constant or even rises if idiosyncratic country-specific shocks are large enough to offset the mean-reverting pull. Danny Quah's (1993) "twin peaks" finding is the empirical version of this critique: he showed the world income distribution became bimodal over the postwar period, with a hollowing-out middle, even in periods and samples where standard growth regressions reported significant negative β coefficients — meaning β-convergence coexisted with a polarizing, not converging, cross-sectional distribution. To support the stronger claim, the researcher must directly compute and track σ, the standard deviation (or another dispersion measure) of log income across the same country panel over time, and show it is declining — a distinct empirical exercise from estimating the growth-on-initial-income regression coefficient.`,
      },
    },
    flashcards: [
      {
    ...
    lessonAssignment: {
      prompt: `Country A records GDP growth of 7% per year, capital stock growth of 11% per year, and labor force growth of 2% per year, with capital's income share α = 0.30. Country B records GDP growth of 4% per year, capital stock growth of 5% per year, and labor force growth of 1% per year, with the same α = 0.30. (a) Compute total factor productivity (TFP) growth for both countries using the Solow residual formula. [4 marks] (b) Using the two TFP figures, compute what fraction of each country's GDP growth is attributable to TFP versus factor accumulation, and explain, referencing the Young/Krugman "perspiration versus inspiration" critique of East Asian growth, why Country A's faster headline growth rate does not necessarily mean its growth is more technologically driven than Country B's. [5 marks] (c) A researcher regresses growth rate on 1990 log GDP per capita across a panel of 100 countries with similar institutional and savings-rate controls and finds a coefficient β = −0.02 (statistically significant), concluding that cross-country income dispersion is falling. Explain precisely what this coefficient does and does not establish, invoking the β-convergence versus σ-convergence distinction. [6 marks] (d) A policymaker wants to determine whether Country B's higher income level is primarily attributable to superior institutions or favorable geography, and proposes constructing a settler-mortality instrument analogous to Acemoglu, Johnson, and Robinson (2001), even though Country B was never a European colony. State the two formal conditions required for any valid instrumental variable, and explain one specific reason a settler-mortality-style instrument would be difficult or invalid to construct for a never-colonized country. [5 marks]`,
      rubric: `(a) 4 marks: correct Country A TFP growth = 7 − 0.30(11) − 0.70(2) = 2.3% (2); correct Country B TFP growth = 4 − 0.30(5) − 0.70(1) = 1.8% (2). (b) 5 marks: correctly computes TFP share for A = 2.3/7 ≈ 32.9% and for B = 1.8/4 = 45% (2); correctly notes that despite A growing faster in absolute terms, a larger proportion of B's growth is TFP-driven, so A's growth is relatively more "perspiration" (factor accumulation) and B's relatively more "inspiration" (efficiency/technology) (2); correctly references that Young (1995) and Collins-Bosworth (1996) found this pattern for East Asian "miracle" economies, undermining a naive equation of high growth with high technological progress (1). (c) 6 marks: correctly explains β-convergence is a claim about the growth-initial-income regression coefficient, not directly about dispersion (2); correctly explains σ-convergence requires directly tracking the cross-sectional standard deviation of log income over time (2); correctly notes Galton's fallacy / Quah's twin-peaks critique — a negative β can coexist with constant or rising σ due to mean reversion in noisy data or large idiosyncratic shocks, so the researcher's conclusion about dispersion is not established by β alone (2). (d) 5 marks: correctly states relevance (instrument must be correlated with the endogenous regressor) and the exclusion restriction (instrument must affect the outcome only through the endogenous regressor) (2); gives a valid reason the analogy fails for a never-colonized country, e.g. no comparable historical settler-mortality records or colonial-institution-formation event exist to instrument the relevant institutional variation, or any surviving disease-burden variable would plausibly violate the exclusion restriction by affecting current productivity directly rather than only through institutions (3).`,
      exampleTopics: [
        'Applying the Solow growth-accounting decomposition to compute and interpret TFP growth',
        '"Perspiration versus inspiration" — distinguishing factor-driven from productivity-driven growth',
        'β-convergence versus σ-convergence and the Galton\'s-fallacy / twin-peaks critique',
        'Formal conditions for instrument validity: relevance and the exclusion restriction',
        'Limits of transplanting an identification strategy (settler mortality) to a different institutional context',
      ],
    },
  },
]
